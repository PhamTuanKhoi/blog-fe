      ...
      <>
               ...
               <div key={item?._id} className="container">
                  ...
                  <div className="title-content">
                     <div className="internation">
                        <div className="inter-name">{item?.user?.name}</div>
                        <div className="inter-date">{item?.user?.dob}</div>
                     </div>
                     <div className="tag">
                        {item?.tags.map((i) => (
                  ...
                  <div className="number-cmt">{item?.cmt?.length} replies</div>
                  <hr />
                  {item?.cmt?.map((i) => (
                     <div key={i?.content} className="title-content cmt-main">
                        <div className="logo">
                           <img
                              style={{ width: "30px" }}
                        ...
                        <div className="cmt">
                           <div className="title-content">
                              <div className="name-cmt">{i?.user?.name}</div>
                              <div>{i?.user?.dob}</div>
                           </div>
                           <div className="main-cmt">{i?.content}</div>
                        </div>
                     </div>
                  ))}
                  {user?._id ? (
         ...
         <Modal show={show} onHide={handleClose}>
            <Modal.Header closeButton>
               <Modal.Title>Cmt</Modal.Title>
            </Modal.Header>
            <Modal.Body>
               <input type="text" className="input-all" onChange={(e) => setCmt(e.target.value)} value={cmt} />
            </Modal.Body>
            <Modal.Footer>
               <Button variant="danger" onClick={handleCmt}>
                  OK
               </Button>
            </Modal.Footer>
         </Modal>
